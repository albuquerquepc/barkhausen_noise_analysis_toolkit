...
def main():
    ...
    get_frequencies = np.linspace(0, int(SAMPLING_RATE/2), int(len_of_1st_file/2+1))
    ...
    np.savetxt(fname=f"/home/paulo/Scripts/barkhausen-noise-analysis/Power_Spectrum_Selected_{INTERVAL_START_IN_SECONDS}s_{INTERVAL_END_IN_SECONDS}s_Py_1000nm_R798D_0.05Hz_100kHz_4MSs/{SAMPLE_ID}{counter:03}.dat", X=power_spectrum_mean, fmt='%.8e')

    plt.loglog(get_frequencies, power_spectrum_mean[0:(int(len_of_1st_file/2+1))])
    plt.show()
